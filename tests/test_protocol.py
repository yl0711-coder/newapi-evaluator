...
class ProtocolTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_live_adapter_uses_workbench_network_guard(self):
        adapter = OpenAIAdapter('https://api.example.test/v1', confirm_live=True)
        try:
            self.assertIsInstance(adapter.transport.inner._pool._network_backend, PublicNetwork)
        finally:
