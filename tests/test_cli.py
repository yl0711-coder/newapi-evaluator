...
class CLITests(unittest.TestCase):
    ...
    def test_config_inspection_safe(self):
        result = self.run_cli('inspect-config', '--config', 'configs/relay-lab/example.yaml')
    ...
    def test_revision_allows_minimal_image_without_git(self):
        from relay_lab.report import revision
        with patch.dict(os.environ, {'RELAY_LAB_IMAGE_REVISION': ''}), patch(
                'relay_lab.report.subprocess.run', side_effect=FileNotFoundError):
            self.assertEqual(revision(), {'commit_sha': None, 'dirty': False})
