...
from relay_lab.network import audit, mock_network_guard
...
class MetricSecurityTests(unittest.TestCase):
    ...
    def test_socket_guard_does_not_affect_other_workbench_threads(self):
        evidence = {'loopback_connections': 0, 'blocked_external_attempts': 0}
        outcome = []
        with mock_network_guard(evidence):
            worker = threading.Thread(
                target=lambda: outcome.append(audit('socket.connect', (None, ('192.0.2.1', 443))))
            )
            worker.start()
            worker.join()
            with self.assertRaises(ValueError):
                audit('socket.connect', (None, ('192.0.2.1', 443)))
    ...
    def test_config_and_output_validation(self):
        ...
        with self.assertRaises(ValueError):
            data_path(DATA_ROOT.parent / 'outside-relay-lab')
