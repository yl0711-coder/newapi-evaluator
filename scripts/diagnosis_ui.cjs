...
(async()=>{
  ...
  app=spawn(process.env.PYTHON_EXECUTABLE,['scripts/start_diagnosis.py','--data-dir',data,'--port',String(port)],{cwd:root,env:{...process.env,DIAGNOSIS_ENABLE_LIVE:'0',PLATFORM_USERNAME:'',PLATFORM_PASSWORD:''},stdio:['ignore','pipe','pipe']});
  ...
  assert.equal(await page.locator('#mode option[value=live]').evaluate(option=>option.disabled),true);
  assert.equal((await (await fetch(base+'/diagnosis/api/config')).json()).live_enabled,false);
