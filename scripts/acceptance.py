...
def main():
    ...
    commands = [
        ('inspect', [sys.executable, '-m', 'relay_lab', 'inspect-config', '--config', 'configs/relay-lab/example.yaml']),
        ...
        ('sustained-cli', [sys.executable, '-m', 'relay_lab', 'account-test', '--config', 'configs/relay-lab/sustained.yaml',
                           '--output', str(output / 'sustained')]),
        ('mixed-burst-cli', [sys.executable, '-m', 'relay_lab', 'account-test', '--config', 'configs/relay-lab/mixed-burst.yaml',
    ...
    lines = ['# 独立测试报告', '', '- PR：由验收人员填写；验收脚本不执行推送或合并。',
             '- 分支：主仓极限测试集成分支；本副本 detached HEAD。',
             f'- 实际验证 SHA（40 位）：`{sha}`',
             f'- Python：{platform.python_version()}；系统：{platform.system()} {platform.machine()}',
             '- 依赖：使用与开发工作树分离的 Python 环境。',
             '- 一键渠道信息：`python -m relay_lab inspect-config --config configs/relay-lab/example.yaml`，仅安全别名、协议、数量、指纹与时间。',
