...
def scan(root):
    ...
    fixture_markers = ('synthetic', 'selftest', 'never-public', 'ephemeral', 'rotated', 'new-test-key', 'inventory')
        ...
        for i, pattern in enumerate(patterns):
            ...
            if i == 0:
                matches = [match for match in matches if not any(
                    marker in match.group(0).casefold() for marker in fixture_markers
                )]
            if matches:
