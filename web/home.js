(async () => {
  ...
  const descriptions = {admission:'临时填写候选端，选择库中的参照端。首轮预热、同题成对评测，查看能力证据、相对速度与异常归因。', stability:'按指定时刻检查选中的模型端点，保存每轮结果，按需发送飞书通知。', reasoning:'单渠道 5 题非流式测试，查看最终答案、可见推理、截断信号和原始响应。'};
