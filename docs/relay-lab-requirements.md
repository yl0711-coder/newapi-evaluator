...
| 要求 | 实现 | 行为验证/产物 |
| --- | --- | --- |
| 集成到现有工作台、共用访问控制与容器、运行数据隔离 | workbench.py、features/capacity/api.py、relay_lab/config.py | test_capacity_integration、容器健康检查与浏览器冒烟测试 |
...
| 工作台说明、统一及六类配置、源代码、自动测试、一键命令 | README、configs/relay-lab/、relay_lab/、features/capacity/、tests/、scripts/ | 仓库清单和独立验收源代码检查 |
...
| 功能提交、detached 验收、准确 40 位 SHA、验收不执行合并或 push | 主仓 feature 分支、独立 worktree | acceptance.json、Git HEAD/branch/status/remote/worktree 检查 |
