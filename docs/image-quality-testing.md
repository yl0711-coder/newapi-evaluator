...
适用规范：根 AGENTS.md 引用的 docs/ai-rules v1.0。测试清单版本 image-quality-3。

## 范围

工作台独有模块 features/image_quality；挂载、导航、独立启动、依赖、合成夹具与测试入口是必要消费者。极限测试引擎源码未更改，仍保留工作台原有全部必需检查。原工作台未提交样式修改不属于本次候选。

集成目录：/Users/lmurder/Desktop/api中转站/模型测试工作台-生图集成
分支：feature/integrate-image-quality
目标：主项目当前分支 codex/unify-workbench-style，基线 b6db4f03953f4dd0af9822824fef62073fb85eb0
生图来源：67b495be68f226cc7e89ea593f21e351494e1456
...
生图原有八组验证（output 必须全新；主项目整合执行下节并集）：
...
主项目同时包含请求特征诊断和生图。整合验收使用两份清单的并集，共享组在同一候选 SHA 下执行一次即可对应两份清单，不减少任何必需场景：

1. 执行 `scripts/verify_diagnosis.py --output 全新外置目录` 的九组：工作台 Python、Web、安全、全量语法、五模式 E2E、工作台浏览器、诊断浏览器、诊断只读检查、诊断容器。工作台浏览器包含六个首页入口、两个新模块的导航、首页宽窄屏，以及生图返回与下载。Python 包含全部生图与诊断测试，并验证六模块同时挂载与分别启动。
2. 另执行下表的 `image-inspect` 与 `build-container` 生图专属组；证据分别登记为 image-inspect、image-container，后者覆盖 all/image-quality 两种模式。直接入口分别为 `python -m features.image_quality inspect-config --config tests/fixtures/image_quality/config.json` 和 `python scripts/image_quality_container.py --output 全新外置目录`，仍实施 60s/900s 上限、退出码/结构化结果与资源审计。
3. 独立验收在干净 detached SHA 上给诊断统一入口加 `--sha 完整SHA`，按原诊断清单额外执行 legacy-acceptance 十组；保留该既有契约所需的重复项。生图专属两组同样独立执行。
...
以下是生图模块原有八组；主项目整合须同时满足上节并集。统一执行器中的同名组为实际注册入口；总预算 30 分钟，每组上限由执行器实施。超时、依赖缺失、零用例、缺失框架结果或必需跳过为 incomplete；已确认断言失败为 failed，逐组记录 failure_count。后续预算耗尽的组为 not_run，不覆盖为通过。容器阶段收到预算终止信号时，在外层 5 秒清理宽限内清理登记的专属容器及镜像；每个清理命令最多 1.5 秒，失败保留资源身份与证据。执行器收到 SIGINT/SIGTERM 时也终止所属进程组，记录当前组取消与剩余组 not_run；确定失败数量和失败状态不会被取消或源码指纹变化覆盖。
...
生图原有组在开发和独立验收环境均适用，共享组按上节同 SHA 映射。类型/格式工具当前未配置，不宣称已执行类型检查。真实上游、实际模型身份、长期实测和生产部署不属于本次 Mock 验收。CI 当前只在版本标签触发部分组；本次不推送、不发 Tag，也不宣称已完成远端 CI。
