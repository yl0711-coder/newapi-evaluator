...
以下是本次代码、页面、依赖与启动配置交付全部适用必需组。统一执行器中的同名组为实际注册入口；总预算 30 分钟，每组上限由执行器实施。超时为 incomplete，后续预算耗尽的组为 not_run，不覆盖为通过。
...
新增用例覆盖固定 gpt-image-2 与精确负载、默认参数、URL/Key/Prompt 输入边界、无确认不触网、无 Key/URL/正文持久化或指标输出、重定向/鉴权/限流/5xx、空或损坏 JSON、URL-only 响应、损坏或错误图片格式、图片元数据移除、响应大小限制、总超时及断连取消、缺失模型/用量语义、工作台鉴权与跨站保护。
