(() => {
  ...
  $('case-file').addEventListener('change',guard(async()=>{ const file=$('case-file').files[0]; if(!file)return; if(file.size>262144)throw Error('文件不能超过 256 KiB'); let data; const text=await file.text(); try {data=JSON.parse(text);} catch {try{data=text.split(/\r?\n/).filter(x=>x.trim()).map(x=>JSON.parse(x));}catch{throw Error('文件不是有效的指标 JSON / JSONL');}} const rows=await post('/cases',Array.isArray(data)?{cases:data}:data&&typeof data==='object'&&Object.prototype.hasOwnProperty.call(data,'cases')?data:{cases:[data]}); await loadCases(rows[0].id); $('case-file').value=''; status(`已导入 ${rows.length} 条案例。`); }));
