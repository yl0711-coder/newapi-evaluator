...
platform_data = os.environ.get('PLATFORM_DATA_DIR', '').strip()
relay_data = os.environ.get('RELAY_LAB_DATA_DIR', '').strip()
PLATFORM_DATA_ROOT = Path(platform_data or PROJECT_ROOT / 'data').expanduser().resolve()
HOST_DATA_ROOT = Path(relay_data or PLATFORM_DATA_ROOT / 'relay-lab').expanduser().resolve()
# The standalone lab image has one fixed writable mount. Integrated workbench runs
# default to the ignored platform data directory and may use an explicit deployment path.
