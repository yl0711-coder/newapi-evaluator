...
class FaultTransport(httpx.AsyncBaseTransport):
    ...
    def __init__(self, limits, enabled, guard_public=False):
        self.inner = guarded_transport(limits=limits) if guard_public else httpx.AsyncHTTPTransport(
            limits=limits, retries=0, trust_env=False
        )
...
class OpenAIAdapter:
    def __init__(self, base_url, *, timeout=0.5, connection_limit=1500, confirm_live=False, api_key=None, model='relay-lab-model', burst_timeouts=None):
        ...
        self.transport = FaultTransport(limits, False, guard_public=confirm_live)
