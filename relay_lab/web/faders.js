...
const FaderUI=(()=>{
  ...
  async function send(){
    ...
    try{
      while(pending){
        ...
        const job=await api(`${API_ROOT}/jobs/`+live.id+'/faders',update);
  ...
  function render(job){
    ...
    $('download-fader-events').hidden=!job.downloads.includes('fader-events.json');$('download-fader-events').href=`${API_ROOT}/jobs/${job.id}/fader-events.json`;
    sync();
  }
