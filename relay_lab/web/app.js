...
function renderJob(job) {
  ...
  [['download-report','report.md'],['download-summary','summary.json'],['download-raw','results.jsonl']].forEach(([id,file])=>{$(id).href=`${API_ROOT}/jobs/${job.id}/${file}`;$(id).setAttribute('download',file);});
}
...
async function selectJob(id) {try{renderJob(await api(`${API_ROOT}/jobs/`+id));await refresh();}catch(e){showError(e.message);}}
async function refresh() {
  if(refreshing)return;refreshing=true;
  try{const state=await api(`${API_ROOT}/state`);token=state.csrf;sustainedSupported=state.ui_schema_version>=2;burstSupported=state.ui_schema_version>=3;fadersSupported=state.ui_schema_version>=4;active=state.jobs.find(j=>['running','stopping'].includes(j.status))?.id||null;
    $('connection').textContent='本地服务已连接';$('local-address').textContent=location.host;
    if(!selected&&active)selected=active;if(selected)renderJob(await api(`${API_ROOT}/jobs/`+selected));renderHistory(state.jobs);busy();
...
$('test-form').addEventListener('submit',async(event)=>{event.preventDefault();showError('');if(active||submitting)return;
  ...
  try{const job=await api(`${API_ROOT}/jobs`,body);$('api-key').value='';active=job.id;renderJob(job);await refresh();}catch(e){showError(e.message);}finally{body.api_key='';submitting=false;busy();}
});
$('stop-button').addEventListener('click',async()=>{if(!selected)return;try{renderJob(await api(`${API_ROOT}/jobs/`+selected+'/stop',{}));}catch(e){showError(e.message);}});
